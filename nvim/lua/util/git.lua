...
---Cai para o cwd quando o buffer não tem arquivo (dashboard, terminal, etc).
---
---Feito em Lua puro com vim.fs.find em vez de `git rev-parse`: não gasta
---um processo e funciona igual para worktree e submódulo, onde `.git` é um
---ARQUIVO apontando para o gitdir de verdade — por isso não filtramos por type.
...
-- Cada forge nomeia as coisas de um jeito. `pr` é pull request no GitHub e
-- Bitbucket, merge request no GitLab.
local FORGES = {
  github = { commit = "/commit/%s", pr = "/pull/%s", linha = "/blob/%s/%s#L%d", nome_pr = "PR" },
  bitbucket = { commit = "/commits/%s", pr = "/pull-requests/%s", linha = "/src/%s/%s#lines-%d", nome_pr = "PR" },
  gitlab = { commit = "/-/commit/%s", pr = "/-/merge_requests/%s", linha = "/-/blob/%s/%s#L%d", nome_pr = "MR" },
}
...
--------------------------------------------------------------------------------
-- PR / MR que introduziu o commit
--------------------------------------------------------------------------------

---Número do pull/merge request que trouxe `sha` para a linha principal.
---
...
---Abre no navegador o PR/MR que introduziu a linha atual.
function M.open_pr()
  local info, erro = M.blame_line()
  if not info then
    return aviso(erro)
  ...
  if not numero then
    aviso(
      ("não achei %s para o commit %s.\nAbrindo o commit no lugar."):format(remoto.forge.nome_pr, info.sha:sub(1, 8)),
      vim.log.levels.INFO
    )
  ...
  vim.notify(
    ("%s #%s (via %s)\ncommit %s — %s"):format(remoto.forge.nome_pr, numero, como, info.sha:sub(1, 8), info.autor),
    vim.log.levels.INFO,
    { title = "abrindo " .. remoto.forge.nome_pr }
  )
